...
output_file = os.path.join(os.path.dirname(__file__), '../raw_outputs/weekly_deliveries.csv')
...
# Process the dataframe to extract the deliveries data
...
for index, row in df.iterrows():
    ...
    # Check for DELIVERED row (but not RETURN TOTAL or other similar rows)
    is_delivered_row = False
    for val in row_values[:3]:  # Check first few columns for the word DELIVERED
        if 'DELIVERED' in val.upper() and not any(exclude in val.upper() for exclude in ['DELIVERED TOTAL', 'TOTAL BEFORE', 'GRAND TOTAL']):
            is_delivered_row = True
            break
    
    if is_delivered_row and current_store and cake_type_positions and current_week:
        print(f"Row {index}: Processing DELIVERED for store: {current_store}")
        
        # Index 3 is usually column D, where the cake type values start
        data_start_index = 3
        
        # Collect delivered data from this row
            ...
            if pos >= data_start_index:  # Skip non-data columns
                ...
                if pd.notna(value) and str(value).strip() != '' and str(value).strip().upper() != 'TOTALS':
                    ...
                    try:
                        ...
                        deliveries.append(numeric_value)
                        delivery_amounts.append(0)  # Will be updated in RETURN TOTAL row
                        print(f"  Added delivery for {cake_type}: {numeric_value}")
                    except (ValueError, TypeError):
                        # Skip non-numeric values
                        pass
    
    # Check for TOTAL BEFORE RETURNS row
    is_return_total_row = False
    for val in row_values[:3]:  # Check first few columns for RETURN TOTAL
        if 'TOTAL BEFORE RETURNS' in val.upper():
            is_return_total_row = True
            break
    
    if is_return_total_row and current_store and cake_type_positions and current_week:
        print(f"Row {index}: Processing TOTAL BEFORE RETURNS for store: {current_store}")
                    ...
                    try:
                        ...
                        for i in range(len(weeks) - 1, -1, -1):
                            if stores[i] == current_store and cake_types[i] == cake_type and weeks[i] == current_week:
                                delivery_amounts[i] = numeric_value
                                print(f"  Updated delivery amount for {cake_type}: {numeric_value}")
                                break
                    except (ValueError, TypeError):
                        # Skip non-numeric values
                        pass
...
deliveries_df = pd.DataFrame({
    ...
    'DeliveryAmount': delivery_amounts
})

# Convert numeric columns
deliveries_df['Delivered'] = pd.to_numeric(deliveries_df['Delivered'], errors='coerce')
deliveries_df['DeliveryAmount'] = pd.to_numeric(deliveries_df['DeliveryAmount'], errors='coerce')

# Remove any rows where either 'Delivered' or 'DeliveryAmount' contains non-numeric values
deliveries_df = deliveries_df.dropna(subset=['Delivered', 'DeliveryAmount'])
...
if len(weeks) == 0:
    ...
    print(f"Successfully extracted {len(deliveries_df)} records with valid numeric values.")
